...
- **“Due by end of today” includes overdue tasks** — they are still due by the end of today. The boundary is `Time.zone.now.end_of_day` with `config.time_zone` set explicitly, so “today” follows the app timezone instead of UTC.
...
- **Attachments** use Active Storage (disk service) with manual size/type validation (≤10 MB; png/jpeg/pdf/txt) since Active Storage ships no built-in validations. Blobs are purged in the background (`dependent: :purge_later`).
