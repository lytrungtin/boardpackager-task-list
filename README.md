...
- `Gemfile.lock` was not regenerated when `bcrypt` was uncommented (this delivery was prepared without a local Ruby toolchain). The dev image runs a non-frozen `bundle install` on boot, which resolves it transparently. To pin it: `docker compose run --rm web bundle lock` and commit the diff. A production image with `BUNDLE_FROZEN=true` would require that step first.
- `db/schema.rb` is generated by the first `db:prepare` run; commit it afterwards if you prefer schema-file-first workflows.
...
- Password reset via mailer; CI pipeline (rubocop + tests) with a production Dockerfile.

## Approximate time log (~8h)

| Block | Time |
|-------|------|
| Docker/Rails skeleton & CI-less setup | 0.5h |
